...
def test_backup_only_starts_during_the_starters_bye():
    ...
    starter = make_player(
        par_per_game=20.0, expected_games_played=17.0, bye=7, name="Starter"
    )
    backup = make_player(
        par_per_game=10.0, expected_games_played=17.0, bye=12, name="Backup"
    )
...
def test_no_available_but_different_bye():
    ...
    drafted_players = [
        make_player(par_per_game=11, bye=7),
        make_player(par_per_game=11, bye=7),
        make_player(par_per_game=11, bye=7),
    ]
...
def test_better_player_starts_fully():
    drafted_players = [
        make_player(par_per_game=1, bye=8),
        make_player(par_per_game=2, bye=12),
    ]
...
def make_roster(starting_positions=None, roster_size=15) -> Roster:
    ...
    return Roster(starting_positions, roster_size=roster_size)
...
def test_all_rbs():
    ...
    rb3_flex = make_player(
        par_per_game=6,
        par_per_game_flex=6,
        position="FLEX",
        bye=3,
        expected_games_played=15,
    )
...
def test_all_multiple_pos():
    ...
    rb2 = make_player(par_per_game=9, position="RB", bye=2)
    rb3 = make_player(
        par_per_game=8, par_per_game_flex=6, position="RB", bye=3, name="flex1"
    )
    wr1 = make_player(par_per_game=10, position="WR", bye=1)
    wr2 = make_player(par_per_game=9, position="WR", bye=2)
    wr3 = make_player(
        par_per_game=7, par_per_game_flex=5, position="WR", bye=3, name="flex2"
    )
    roster.drafted["RB"] = [rb1, rb2, rb3]
    roster.drafted["RB"].sort(reverse=True)
...
def test_all_multiple_pos2():
    ...
    rb2 = make_player(par_per_game=9, position="RB", bye=2)
    rb3 = make_player(
        par_per_game=7, par_per_game_flex=5, position="RB", bye=3, name="flex1"
    )
    wr1 = make_player(par_per_game=10, position="WR", bye=1)
    wr2 = make_player(par_per_game=9, position="WR", bye=2)
    wr3 = make_player(
        par_per_game=8, par_per_game_flex=6, position="WR", bye=3, name="flex2"
    )
    roster.drafted["RB"] = [rb1, rb2, rb3]
    roster.drafted["RB"].sort(reverse=True)
    roster.drafted["WR"] = [wr1, wr2, wr3]
...
def test_get_player_pos_num_walks_down_the_position_ranking():
    frame = drafted_frame(simple_partable())
    assert [get_player(0, "RB", i, frame).name for i in range(4)] == [
        "RB1",
        "RB2",
        "RB3",
        "RB4",
...
def test_get_player_skips_players_gone_before_this_pick():
    ...
    frame = drafted_frame(simple_partable()).with_columns(
        pl.when(pl.col("player") == "RB1")
        .then(2)
        .otherwise(pl.col("drafted_after"))
        .alias("drafted_after")
    )
    assert get_player(0, "RB", 0, frame).name == "RB1"
    ...
    partable = make_partable(
        [
            make_raw_row(
                ...
                "RB",
                18.0,
                replacement_ppg=10.0,
                expected_games_played=15.0,
                bye=9,
            )
        ]
    )
...
def test_draft_combos_only_drafts_allowed_positions():
    partable = make_partable(
        [
            make_raw_row("RB1", "RB", 20),
            make_raw_row("RB2", "RB", 19),
            make_raw_row("K1", "K", 12),
            make_raw_row("DEF1", "DST", 11),
        ]
    )
    combos = collect_combos(
        get_draft_combos(
            ...
            partable,
            consider_per_position=1,
            position_ignore={"K": 99, "DEF": 99, "TE": 99, "QB": 99, "WR": 99},
        )
    )
...
def test_draft_combos_respects_drafted_after():
    ...
    partable = simple_partable().with_columns(
        pl.when(pl.col("player").is_in(["RB1", "WR1"]))
        .then(2)
        .otherwise(pl.col("drafted_after"))
        .alias("drafted_after")
    )
...
def test_draft_combos_can_track_the_best_combo_by_par():
    ...
    best = max(
        combos,
        key=lambda combo: sum(p.par_per_game * p.expected_games_played for p in combo),
    )
...
def test_draft_combos_falls_back_when_a_position_is_exhausted():
    ...
    partable = make_partable(
        [make_raw_row("RB1", "RB", 20), make_raw_row("WR1", "WR", 19)]
    )
    combos = collect_combos(
        get_draft_combos(
            [0, 1],
...
def rb_wr_partable() -> pl.DataFrame:
    ...
    return make_partable(
        [
            ...
            make_raw_row("RB2", "RB", 18, bye=2),
            make_raw_row("WR1", "WR", 21, bye=3),
            make_raw_row("WR2", "WR", 19, bye=4),
        ]
    )
...
def test_best_draft_sequence_maximises_roster_par():
    ...
    best = best_draft_sequence(
        0, SNAKE_ORDER, roster, partable, through_round=2, position_ignore=ONLY_RB_WR
    )
...
def test_best_draft_sequence_bench_penalty_changes_the_pick():
    ...
    def best_at(bench_penalty: float) -> list[str]:
        ...
        roster.drafted["RB"] = [
            Player(20.0, 16.0, 20.0, 1, "RBstar", "RB"),
...
def test_position_priority_uses_the_rosters_own_starting_positions():
    roster = Roster(
        {"QB": 3, "RB": 2, "WR": 2, "TE": 1, "FLEX": 1, "K": 1, "DEF": 1}, 15
    )
...
def test_draft_priority_pick_takes_the_earliest_adp_among_top_priority_positions():
    ...
    assert make_priority_based_pick(
        priority_roster(),
        adp_partable(),
        "drafted_after",
        False,
...
def test_draft_priority_pick_restricts_itself_to_the_highest_priority_positions():
    ...
    priorities = {
        pos: get_position_priority(pos, roster)
...
def test_full_draft_highest_expected_draft_strategy():
    ...
    """
    partable = prepare_partable(pl.read_csv("outputs/par_table.csv"))
    ...
    draft = full_draft(
        partable,
        {i: only_strategy for i in range(NUM_TEAMS)},
        num_teams=NUM_TEAMS,
